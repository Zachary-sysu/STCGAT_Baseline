...
def compute_val_loss_mstgcn(net, val_loader, adj, criterion,  masked_flag,missing_value,sw, epoch, limit=None):
    '''
...
def predict_and_save_results_mstgcn(net, data_loader, adj, data_target_tensor, global_step, metric_method,_mean, _std, params_path, type):
    '''
...
def predict_and_save_results_mstgcn_1(net, data_loader, data_target_tensor, device,_mean, _std):
    '''
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            input.append(encoder_inputs[:, :, 0:1].cpu().numpy())  # (batch, T', 1)
            outputs = net(encoder_inputs.transpose(2, 3))
            prediction.append(outputs.detach().cpu().numpy())
        ...
        print('prediction:', prediction.shape)
        print('data_target_tensor:', data_target_tensor.shape)
        output_filename = os.path.join('./data/PEMS03', 'output_epoch_%s_%s' % (99, type))
        np.savez(output_filename, input=input, prediction=prediction, data_target_tensor=data_target_tensor)
        ...
        for i in range(prediction_length):
            assert data_target_tensor.shape[0] == prediction.shape[0]
            print('current epoch: %s, predict %s points' % (99, i))
            if metric_method == 'mask':
                mae = masked_mae_test(data_target_tensor[:, :, i], prediction[:, :, i],0.0)
                rmse = masked_rmse_test(data_target_tensor[:, :, i], prediction[:, :, i],0.0)
                mape = masked_mape_np(data_target_tensor[:, :, i], prediction[:, :, i], 0)
