...
files = {
    'pems03': ['PEMS03/PEMS03.npz', 'PEMS03/distance.csv', 'PEMS03/PEMS03.txt'],
    'pems04': ['PEMS04/PEMS04.npz', 'PEMS04/distance.csv', None],
    'pems07': ['PEMS07/PEMS07.npz', 'PEMS07/distance.csv', None],
    'pems08': ['PEMS08/PEMS08.npz', 'PEMS08/distance.csv', None],
...
def read_data(args):
    ...
    if not os.path.exists(f'data/{filename}_spatial_distance.npy'):
        if file[2] is not None:
            with open(filepath + file[2], 'r') as f:
                id_dict = {int(i): idx for idx, i in enumerate(f.read().strip().split('\n'))}  # 把节点id（idx）映射成从0开始的索引

            with open(filepath + file[1], 'r') as f:
                ...
                for row in reader:
                    ...
                    dist_matrix[id_dict[j], id_dict[i]] = distance
    ...
    return data, mean_value, std_value, dtw_matrix, sp_matrix
...
class MyDataset(Dataset):
    def __init__(self, data, split_start, split_end, his_length, pred_length, mean_value, std_value):
    ...
    def __getitem__(self, index):
        data_x = (self.data[index: index + self.his_length] - self.mean_value) / self.std_value
        data_x = torch.from_numpy(data_x.astype(np.float32))
        x = data_x.permute(1, 0, 2)
        # x = self.data[index: index + self.his_length].permute(1, 0, 2)
        data_y = self.data[index + self.his_length: index + self.his_length + self.pred_length]
        data_y = torch.from_numpy(data_y.astype(np.float32))
        y = data_y[:,:, 0].permute(1, 0)
        return torch.Tensor(x), torch.Tensor(y)
    def __len__(self):
        return self.data.shape[0] - self.his_length - self.pred_length + 1


def generate_dataset(data, mean_value, std_value, args):
    """
    Args:
    ...
    train_dataset = MyDataset(data, 0, data.shape[0] * train_ratio, his_length, pred_length, mean_value, std_value)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    valid_dataset = MyDataset(data, data.shape[0]*train_ratio, data.shape[0]*(train_ratio+valid_ratio), his_length, pred_length, mean_value, std_value)
    valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=True)

    test_dataset = MyDataset(data, data.shape[0]*(train_ratio+valid_ratio), data.shape[0], his_length, pred_length, mean_value, std_value)
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True)
    mean = mean_value.reshape(-1)[0]
    std = std_value.reshape(-1)[0]
    return train_dataloader, valid_dataloader, test_dataloader, mean, std
